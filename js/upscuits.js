...
function jsonUptimeRobotApi(data) {
	for (var i in data.monitors.monitor) {
			if(data.monitors.monitor[i].url == "http://blog.eqoe.cn"){
				data.monitors.monitor[i].ServName =  "前端(负载均衡)";
				data.monitors.monitor[i].srid = 2;
			}else if(data.monitors.monitor[i].url == "https://secure.eqoe.cn"){
				data.monitors.monitor[i].ServName =  "后端(负载均衡)";
				data.monitors.monitor[i].srid = 3;
			}else{
				data.monitors.monitor[i].ServName =  "CDN节点";
				data.monitors.monitor[i].srid = 1;
			}
